// # Arrays: Introduction

// ** Instructions **
// 1. Declare a variable named`myArray` and store an array that contains 5 elements.Print the array.
const myArray = ["shoes", "jacket", "pants", "sweater ", "t-shirt"];
console.log(myArray);
// console.log(myArray.length);

// 2. Access the 3rd element in the array and save that to another variable named`thirdElement`.Print the`thirdElement`. 
const target1 = myArray[2];
const target2 = myArray[1]
console.log(myArray);

// console.log(myArray[2]);

// myArray = [2] = "thirdElement";
// console.log(myArray);

// console.log(myArray.length);

// console.log([2] = "thirdElement");

// 3. Reassign the fourth element of the array and give it a value of "fourth element".Print the array once more.
const target1 = myArray[3]; // Helper variable 1
const target2 = myArray[3]; // Helper variable 2
myArray[3] = target2;
myArray[3] = target1;// console.log(fruits); // what in array?
// console.log(myArray[3]);
